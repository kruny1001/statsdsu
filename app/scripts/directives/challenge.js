...
/**
 * @ngdoc directive
 * @name statsdsuApp.directive:challenge
 * @description
 * # challenge
 */
angular.module('statsdsuApp')
  .directive('challenge', function ($timeout, SECArray) {
    return {
      templateUrl:'views/templates/superEditor/challenge.html',
      restrict: 'E',
      scope: {index:'@', mode:'@', cnt:'='},
      link: {
            ...
            for (var i = 0; i < code_tags.length; i++) {
              if ("type" in code_tags[i].dataset) {
                ...
                if (type === "pre-exercise-code") {
                  ...
                  result_object["pre-exercise-code"] = stripIndent(scope.SEC.pre);
                }
                else if (type === "sample-code") {
                  result_object["sample-code"] = stripIndent(scope.SEC.sample);
                }
                else if (type === "solution") {
                  result_object["solution"] = stripIndent(scope.SEC.solution);
                }
                else if (type === "sct") {
                  result_object["sct"] = stripIndent(scope.SEC.sct);
                }
                else if (type === "hint") {
